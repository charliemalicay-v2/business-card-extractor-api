...
- [x] 3.2 Implement `OcrService`
  ...
  - _Status: Done — `app/services/ocr_service.py` + `tests/test_ocr_service.py` (2 tests). Unit tests still monkeypatch `pytesseract.image_to_string` (correct for unit isolation)._
  - _Real-dependency verification (WSL Ubuntu-26.04, 2026-07-27): installed real Tesseract 5.5.0 via apt and ran `OcrService.extract_text` against an actual rendered business-card image (text drawn via `cv2.putText`, not a fixture that was ever intended to carry real text). Output: `'Jane Doe\n\nSales Manager\nAcme Corp\n\njane@acme.com\n+1-555-0100'` — correct extraction, confirming `pytesseract` integration and preprocessing pipeline work against a real OCR engine, not just the mock._
...
- [x] 5.1 Set up llama.cpp model loading and GBNF grammar
  ...
  - _Status: Done — `app/services/llm/model.py` (lazy-imports `llama_cpp` inside `LlamaCppModel.__init__` so the module has no hard import-time dependency on it), `app/services/llm/grammar.gbnf`, and a FastAPI `lifespan` hook in `app/main.py` that loads the model once at startup and degrades gracefully (logs a warning, app still serves requests) if loading fails. **Caveat originally noted here**: `llama-cpp-python` only ships as a source distribution and needs a C++ toolchain/cmake to build, which wasn't available in the original dev sandbox — the GBNF grammar was never validated against a real binding/model._
  - _Real-dependency verification (WSL Ubuntu-26.04, 2026-07-27): installed `cmake`/`gcc`/`g++` via apt, built `llama-cpp-python` from source successfully (no prebuilt wheel needed once a compiler was present), downloaded `Qwen2.5-3B-Instruct-Q4_K_M.gguf` (~2GB, from Hugging Face). **Found and fixed a real grammar bug**: the original `grammar.gbnf` spread the `root` rule across multiple lines using bare line-continuation, which llama.cpp's GBNF parser rejects outside of parentheses (`error parsing grammar: expecting name at ...`) — confirmed by inspecting the official `json.gbnf` example, which only continues across lines inside explicit `(...)` groups. Fixed by collapsing the `root` rule onto a single line. After the fix, real inference against the real model correctly extracted all 5 fields with valid JSON conforming to the grammar in ~17s per call. One non-blocking model-behavior quirk observed: the model sometimes populates `optional_fields` with keys whose value is the literal string `"null"` rather than omitting them — a prompt-tuning opportunity, not a code defect._
...
- [x] 8.1 Implement `POST /cards` endpoint
  ...
  - _Status: Done — `app/api/cards.py`, `app/api/dependencies.py`, `app/api/exception_handlers.py`, `app/schemas/response.py`. Two real bugs caught and fixed while writing the integration tests (not left as TODOs): (1) `Depends(get_card_processing_service)` resolved eagerly before the endpoint body ran, calling `get_model()` unconditionally — meaning content-type/size validation would never be reached whenever the LLM wasn't loaded, always returning 503 first. Fixed with a `_LazyLlmModel` wrapper in `app/api/dependencies.py` that defers the `get_model()` call until inference is actually invoked inside `process()`, after validation and classification. (2) `HTTPException(detail={...})` responses were nested under `{"detail": {...}}` by FastAPI's default handling, not matching the flat `{error_code, message}` shape the typed-exception handlers and the design's `ErrorResponse` schema use — fixed with a generic `HTTPException` handler in `exception_handlers.py` that flattens dict `detail` to the top level. Also had to install `python-multipart` (already listed in `requirements.txt` from task 1.1 but not yet installed in the working venv) since FastAPI raises at import time without it once a `File(...)` param exists._
  - _Update (real-dependency verification pass): the 503 test originally relied on llama-cpp-python being **absent** from the sandbox to genuinely trigger `ExtractionServiceUnavailableError` via a real `ImportError`. Once llama-cpp-python was actually installed and a real GGUF model wired in (see the note under task 5 below), that test started silently returning 201 instead of 503 — an environment-fragile test, not a real code bug. Fixed by rewriting the test to force unavailability via a fake model that raises, so it passes deterministically regardless of what's installed on the machine running it._
...
- [x] 9.2 Validate performance budget
  ...
  - _Status: Done — `tests/test_performance.py`. Measured non-OCR/non-LLM pipeline overhead (image decode/preprocess, shape classification, real QR decode, reconciliation, real Postgres write) at 5 fixture runs: 23–87ms, comfortably within the 30s budget. This automated test still fakes OCR/LLM (kept fast and deterministic for CI); the real end-to-end number below was gathered by manual verification, not by this test file._
  - _Real end-to-end measurement (WSL Ubuntu-26.04, 2026-07-27, manual, not part of the automated suite): ran the full `CardProcessingService.process()` pipeline with **real** Tesseract OCR, real QR check, real `Qwen2.5-3B-Instruct-Q4_K_M` llama.cpp inference (CPU), and a real Postgres write, end to end. **Total: 15.8s** (real image → OCR ~0.1s territory → LLM inference ~16s dominant cost, as the design predicted → persist), comfortably within the 30s budget on this machine. This closes out the "lower bound, not a real number" caveat from the original performance-test note — the actual bottleneck is confirmed to be LLM inference, exactly as the design document anticipated. Note this is one CPU, one model, one run; production capacity planning should still benchmark against target hardware and expected concurrent load._
