...
- [x] 5. Local image-serving endpoint
- [x] 5.1 Add `GET /cards/{id}/image` to `app/api/cards.py`
  ...
  - _Requirements: 2.3_
- [x] 5.2 API tests: 200 with correct bytes/content-type for an existing local image; 404 for
  missing record, missing image, and non-local backend
  - _Requirements: 2.3, 2.5_

- [x] 6. Update endpoint (`PATCH /cards/{id}`)
- [x] 6.1 Add `CardUpdateRequest` schema (all fields optional) to `app/schemas/response.py` (or a
  new `app/schemas/request.py` if cleaner alongside existing schemas)
  - _Requirements: 4.1, 4.4_
- [x] 6.2 Add `CardRepository.update(card_id, fields: dict) -> BusinessCardRecord`
  - Applies field updates, commits, refreshes, relies on existing `onupdate=func.now()` for
    `updated_at`
  - _Requirements: 4.1, 4.6_
- [x] 6.3 Implement `PATCH /cards/{id}` route
  ...
  - `optional_fields` is sent as a JSON-encoded string form field (multipart forms can't carry
    nested objects); invalid or non-object JSON is rejected as `400 invalid_update_payload`
  - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.7_
- [x] 6.4 API tests for `PATCH /cards/{id}`
...
- [x] 7. Delete endpoint (`DELETE /cards/{id}`)
- [x] 7.1 Add `CardRepository.delete(card_id) -> None`
  - Implemented as `-> bool` (mirrors `update`/`resolve_review`'s not-found handling
    convention); route still does its own upfront `get_by_id` check before deciding whether
    to attempt an `image_storage.delete()`
  - _Requirements: 5.1_
- [x] 7.2 Implement `DELETE /cards/{id}` route
...
- [x] 7.3 API tests for `DELETE /cards/{id}`
